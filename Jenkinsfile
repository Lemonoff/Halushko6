...
def CONTAINER_ARGS_MAP=[
"bot-container-1":
["client","1021831296:AAGrOQA3-IZJcQBPl1uuMgG6WY99QuO91Q0", "LemRazBot"],
"bot-container-2":
["client","1277739404:AAH_CPeDaY2se3Fdg8RERjqW82qPl4DfwCI", "LemRaz2Bot"]]
def IMAGE_TAG="latest"
def DOCKER_HUB_USER="artemissio"
node {
stage('Initialize'){
}
stage('Checkout') {
...
/*Зупинити та видалити контейнери, які використовували застаріле зображення, та
запустити ботів на контейнерах з оновленим зображенням*/
stage('Run App'){
CONTAINER_NAMES.each{ item ->
runApp(item, CONTAINER_ARGS_MAP.get(item).get(0),
CONTAINER_ARGS_MAP.get(item).get(1), CONTAINER_ARGS_MAP.get(item).get(2), IMAGE_NAME,
IMAGE_TAG, DOCKER_HUB_USER)
}
}
...
sh "docker run -d --rm --network host --name $containerName
$dockerHubUser/$imageName:$tag $arg0 $arg1 $arg2"
